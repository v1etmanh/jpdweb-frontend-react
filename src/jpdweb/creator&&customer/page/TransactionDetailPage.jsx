...
export default function TransactionDetailPage() {
  ...
  useEffect(() => {
    ...
    const info = {
      ...
      paymentMethod: params.get('paymentMethod') || 'paypal'
    };
    setOrderInfo(info);
  ...
  const currentMethod = paymentMethodInfo[orderInfo.paymentMethod] || paymentMethodInfo.paypal;

  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-600 via-blue-600 to-indigo-700 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full overflow-hidden animate-fadeIn">
        {/* Header */}
        <div className={`bg-gradient-to-r ${currentMethod.color} text-white p-8 text-center`}>
          <CheckCircle className="w-16 h-16 mx-auto mb-4" />
          <h1 className="text-3xl font-bold mb-2">Giao dịch đã được tạo!</h1>
          <p className="text-purple-100">{currentMethod.description}</p>
        </div>
        ...
        <div className="p-8">
          <div className="bg-gray-50 rounded-xl p-6 mb-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between py-2 border-b border-gray-200">
                ...
                <span className="text-2xl font-bold text-purple-600">₫{parseFloat(orderInfo.amount).toLocaleString()}</span>
              </div>
          ...
          <div className="bg-blue-50 border-l-4 border-blue-600 p-6 rounded-lg mb-6">
            ...
            <ol className="space-y-2 text-gray-700 text-sm">
              <li className="flex items-start">
                ...
                <span>Nhấn vào nút "Chuyển đến {currentMethod.name}" bên dưới</span>
              </li>
              <li className="flex items-start">
          ...
          <a 
            ...
            className={`block w-full bg-gradient-to-r ${currentMethod.color} text-white py-4 rounded-xl font-bold text-lg hover:opacity-90 transition-all transform hover:scale-105 shadow-lg hover:shadow-xl text-center`}
          >
            <div className="flex items-center justify-center">
              <ExternalLink className="w-6 h-6 mr-2" />
              Chuyển đến {currentMethod.name} để thanh toán
            </div>
          </a>
          ...
          <div className="mt-6 text-center text-xs text-gray-500">
            <p className="flex items-center justify-center">
              ...
              Giao dịch được bảo mật bởi {currentMethod.name}
            </p>
          </div>
